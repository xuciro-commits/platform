...
func TestHelpdeskTriage(t *testing.T) {
	...
	model := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case 4:
			...
			name, args = "helpdesk_ticket_reply", map[string]any{"target": ticket, "reply": reply}
		}
	...
	ticket := func(id string) helpdesk.Ticket {
		v, _ := w.tenant.RecordOf(lead, helpdesk.TicketType, id, now)
		x, _ := v.Record.(helpdesk.Ticket)
		return x
	}
	...
	instance := func(id string) platformserver.FlowInstance {
		v, _ := w.tenant.RecordOf(flowAdmin, platformserver.InstanceType, "helpdesk.service-level:"+id, now)
	...
	w.expect(do(ops, platformserver.PlatformApp, platformserver.SchemaEndpointAdd, platformserver.EndpointType, "mail-gateway",
		map[string]any{"url": gateway.URL, "secret": "hook", "effects": []string{"helpdesk/" + helpdesk.EffectReply}, "allowPrivate": true}), "ok")
	...
	open := func(id, subject string) {
		w.t.Helper()
		w.expect(do(desk, "helpdesk", helpdesk.SchemaOpen, helpdesk.TicketType, id,
	...
	w.expect(do(lead, "helpdesk", helpdesk.SchemaReply, helpdesk.TicketType, "T-3", map[string]string{"reply": "Corrected, sorry."}), "ok")
	work(3 * time.Second)
