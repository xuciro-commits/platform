...
func TestGroupStayFlow(t *testing.T) {
	...
	win := func(opp string, rooms int, arrive, depart string) {
		t.Helper()
		if opp != "OPP-1" {
			w.expect(w.submit("sales", crm.Authority, crm.SchemaOpen, crm.OpportunityType, opp, "open-"+opp, map[string]string{"account": "ACME", "title": "Group " + opp}), "ok")
		}
		w.expect(w.submit("sales", crm.Authority, crm.SchemaPlan, crm.OpportunityType, opp, "plan-"+opp,
			map[string]any{"rooms": rooms, "roomType": "suite", "arrive": arrive, "depart": depart}), "ok")
		w.expect(w.submit("sales", crm.Authority, crm.SchemaClose, crm.OpportunityType, opp, "won-"+opp, map[string]string{"outcome": "won"}), "ok")
	}
