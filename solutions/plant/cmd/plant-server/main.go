// Command plant-server runs the plant solution — the MES and its books meeting
...
func main() {
	...
	for app, role := range map[string]string{erp.ID: erp.Controller, erplink.ID: erplink.Planner, platformserver.PlatformApp: platformserver.Admin,
	...
	assistant := plant.Seat("assistant-l1", "agent-l1", seat(mes.Assistant), "L1")
	...
	seats := deployment.Seats([]platformserver.Seat{
		...
		plant.Seat("gateway-l1", "gateway-l1", seat(mes.Gateway)),
		assistant,
		plant.Seat("erp", "erp", map[string]string{erplink.ID: erplink.Connector}),
	})
	var app platform.App = erp.New(tenant)
	switch *books {
	case "app":
		deployment.Seed = func(t *platformserver.Tenant, now time.Time) error { return plant.Seed(t, "sup-1", now) }
	case "external":
		app = erplink.New(tenant)
	default:
		log.Fatalf("-erp: %q is neither app nor external", *books)
	}
	t, err := plant.NewTenant(tenant, app, seats...)
	if err == nil && *books == "external" {
		err = t.Connect(erplink.Poll("erp"))
	}
	if err == nil {
