...
func TestProductionThroughTheProtocol(t *testing.T) {
	...
	make := func(shop, mo string, qty, sfcs int) {
		t.Helper()
		expect("release "+shop, do("sup-1", mes.Authority, mes.SchemaRelease, mes.OrderType, shop,
		...
		for n := 1; n <= sfcs; n++ {
			for _, resource := range []string{"FURNACE-1", "CNC-11", "CMM-1"} {
				...
				expect("start", do("op-l1", mes.Authority, mes.SchemaStart, mes.SFCType, sfc, map[string]string{"resource": resource}), "ok")
				expect("complete", do("op-l1", mes.Authority, mes.SchemaComplete, mes.SFCType, sfc, map[string]string{}), "ok")
			}
		}
	}

	expect("MO-1", do("sup-1", erp.ID, erp.ProductionType+".create", erp.ProductionType, "MO-1", map[string]any{"product": "P-100", "quantity": 4}), "ok")
	expect("a draft is no planned order", do("sup-1", mes.Authority, mes.SchemaRelease, mes.OrderType, "SO-0",
	...
	expect("more than planned", do("sup-1", mes.Authority, mes.SchemaRelease, mes.OrderType, "SO-0",
