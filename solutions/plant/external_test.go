...
var externalSeats = []platformserver.Seat{
	Seat("sup", "sup-1", map[string]string{"mes": string(mes.Supervisor), erplink.ID: erplink.Planner, platformserver.PlatformApp: platformserver.Admin,
	...
	func() platformserver.Seat { // the line's AI assistant, a planner in the adapter too
		s := Seat("asst", "agent-l1", map[string]string{"mes": string(mes.Assistant), erplink.ID: erplink.Planner}, "L1")
		s.Agent = true
		return s
	}(),
	Seat("erp", "erp", map[string]string{erplink.ID: erplink.Connector}),
}

func buildExternal(t *testing.T) *platformserver.Tenant {
	tn, err := NewTenant(tenant, erplink.New(tenant), externalSeats...)
	if err == nil {
		err = tn.Connect(erplink.Poll("erp"))
	}
	if err != nil {
		t.Fatal(err)
	}
...
func newExternal(t *testing.T) *external {
	...
	fake := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		x.calls++
		var c struct{ Data erplink.Confirmation }
...
func (x *external) poll(from, to string, orders ...erplink.Planned) {
	x.t.Helper()
	raw, _ := json.Marshal(erplink.Page{CursorFrom: from, CursorTo: to, Orders: orders})
...
func (x *external) make(shop, planned string, quantity, sfcs, scrapped int) {
	x.t.Helper()
	x.expect("release "+shop, x.do("sup-1", mes.Authority, mes.SchemaRelease, mes.OrderType, shop,
		map[string]any{"product": "P-100", "quantity": quantity, "sfcs": sfcs, "planned": planned}), "ok")
	for n := 1; n <= sfcs; n++ {
		sfc := fmt.Sprintf("%s-%03d", shop, n)
		if n > sfcs-scrapped {
			x.expect("nc", x.do("op-l1", mes.Authority, mes.SchemaNC, mes.SFCType, sfc, map[string]string{"code": "POROSITY"}), "ok")
			x.expect("reviewed", x.do("qa-1", mes.Authority, mes.SchemaSign, mes.SFCType, sfc, map[string]string{"action": "scrap", "meaning": "reviewed"}), "ok")
			x.expect("approved", x.do("qa-2", mes.Authority, mes.SchemaSign, mes.SFCType, sfc, map[string]string{"action": "scrap", "meaning": "approved"}), "ok")
			continue
		}
		for _, resource := range []string{"FURNACE-1", "CNC-11", "CMM-1"} {
			x.expect("start", x.do("op-l1", mes.Authority, mes.SchemaStart, mes.SFCType, sfc, map[string]string{"resource": resource}), "ok")
			x.expect("complete", x.do("op-l1", mes.Authority, mes.SchemaComplete, mes.SFCType, sfc, map[string]string{}), "ok")
		}
	}
}
...
func TestConfirmedToAnERPOutside(t *testing.T) {
	x := newExternal(t)
	x.poll("", "page-1", erplink.Planned{ID: "PO-9001", Product: "P-100", Quantity: 4}, erplink.Planned{ID: "PO-9004", Product: "P-100", Quantity: 1})
	...
	resend := func(who, planned string) string {
		return x.do(who, mes.Authority, mes.SchemaResend, mes.OrderType, "SO-2", map[string]string{"planned": planned})
	}
	x.expect("an operator", resend("op-l1", ""), "ERROR_CODE_POLICY_DENIED")
	x.expect("confirmed already", x.do("sup-1", mes.Authority, mes.SchemaResend, mes.OrderType, "SO-1", map[string]string{}), "ERROR_CODE_CONFLICT")
	x.expect("unknown", resend("sup-1", "PO-404"), "ERROR_CODE_INVALID_ARGUMENT")
	x.poll("page-1", "page-2", erplink.Planned{ID: "PO-9002", Product: "P-100", Quantity: 1}, erplink.Planned{ID: "PO-9005", Product: "P-200", Quantity: 1})
	...
	x.expect("too few", x.do("sup-1", mes.Authority, mes.SchemaRelease, mes.OrderType, "SO-9",
	...
	x.poll("page-2", "page-3", erplink.Planned{ID: "PO-9003", Product: "P-100", Quantity: 1})
	x.expect("the assistant resends", x.do("agent-l1", mes.Authority, mes.SchemaResend, mes.OrderType, "SO-3", map[string]string{"planned": "PO-9003"}), "ok")
	...
	x.expect("asked to approve", admin.([]platform.Notification)[0].Title, "Approve Production order confirmation for erplink.order/PO-9003")
...
func TestCorrectedByTheAgent(t *testing.T) {
	...
	x.poll("", "page-1", erplink.Planned{ID: "PO-9001", Product: "P-100", Quantity: 1}, erplink.Planned{ID: "PO-9002", Product: "P-100", Quantity: 1})
	x.expect("SO-1", x.do("sup-1", mes.Authority, mes.SchemaRelease, mes.OrderType, "SO-1", map[string]any{"product": "P-100", "quantity": 1, "sfcs": 1, "planned": "PO-9001"}), "ok")
	x.make("SO-2", "", 1, 1, 0)
	x.work(8)
