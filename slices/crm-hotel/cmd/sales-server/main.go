...
var demo = []platformserver.Seat{
	seat("sales", "sales-1", map[string]string{"crm": "sales", "crm-hotel": "sales", "hotel": "front-desk"}),
	seat("sales-only", "sales-2", map[string]string{"crm": "sales", "crm-hotel": "sales"}),
	seat("manager", "manager-1", map[string]string{"crm": "sales-manager", "crm-hotel": "sales-manager", "hotel": "manager", "platform": "admin"}),
...
func main() {
	...
	flag.Parse()
	t, err := crmhotel.NewTenant("hotel-a", map[string]hotel.RoomType{"standard": {Rooms: 3, Overbooking: 1}, "suite": {Rooms: 1}}, deployment.Seats(demo)...)
	if err == nil {
