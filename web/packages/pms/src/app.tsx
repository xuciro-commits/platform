...
function Reservations() {
  const reservations = useRead<{ records: Reservation[] }>("/v1/records/pms.reservation?sort=checkIn,id&archived=true&limit=500")?.records ?? [];
  const openRecord = useOpenRecord();
  return (
    <>
      <PageHeader title={t("Reservations")} description={t("Every stay, from the front desk, the channel and the lodging protocol.")} />
      <ReservationTable data={reservations} onOpen={(r) => openRecord({ type: "pms.reservation", id: r.id })} />
    </>
  );
}

function ReservationDetail({ id }: { id: string }) {
  const r = useRead<{ record: Reservation }>(`/v1/records/pms.reservation/${encodeURIComponent(id)}`)?.record;
...
export default defineApp({
  ...
  views: [
    ...
    { id: "room-types", title: () => t("Room types"), render: () => <Records type="pms.room-type" /> },
  ],
