...
import { z } from "zod";
import { ReservationCard, ReservationTable, newReservation, roomTypeOptions, stay } from "@pkg/pms";
...
function Outbox() {
  ...
  const columns: ColumnDef<OutboxEntry, any>[] = [
    { accessorKey: "schema", header: "Decision", meta: { width: 100 }, cell: (c) => String(c.getValue()).replace("pms.reservation.", "") },
...
export function App() {
  ...
  return (
    <DeskContext.Provider value={{ snapshot, run, startCreate: () => setCreating(true), revise: setRevising }}>
      <Workspace product="Hotel Desk" storageKey="pms-desk.layout" views={views} home={{ view: "reservations" }}
