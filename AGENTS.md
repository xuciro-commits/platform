...
| `apps/erplink/` | The adapter to an ERP outside, such as SAP (ADR-0024 7d): provides `production.orders/1` from the ERP's polled planned orders and sends confirmations to it as effects |
...
| `solutions/plant/` | The plant solution (ADR-0024): platform, the MES and its books — the ERP app, or the adapter to an ERP outside — meeting through `production.orders/1`; `cmd/plant-server` (`-erp external` for the adapter) |
| `apps/drills/` | Evolution drills that run on the kernel alone (E2 shared library) |
| `deploy/local/` | Infrastructure as code for the production path (ADR-0007): Docker Compose with PostgreSQL, Rauthy (declarative bootstrap), plant-server and sales-server; `rehearse.sh` rehearses OIDC principals, restart and restore; `README.md` is the owner's test guide (addresses, accounts, service accounts, what to enter to connect webhooks, email, the ERP and AI providers) |
