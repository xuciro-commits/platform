...
| `apps/erp/` | ERP app (ADR-0024): chart of accounts, journal entries posted with gapless numbers, reversal, periods, the trial balance; partners, products at standard cost, purchase orders with approval, receipts and bills, stock moves and on hand; `cmd/erp-server` is its development host; production orders with the MES follow |
