#!/usr/bin/env bash
# Demo data for the local sales host (deploy/local, OIDC on): customers, an
...
set -euo pipefail
IDP=${IDP:-http://localhost:8480/auth/v1} SALES=${SALES:-http://localhost:8495}
token() {
...
submit() { # token authority key schema target-type target-id payload
  local who body
  who=$(curl -sf -H "Authorization: Bearer $1" "$SALES/v1/me" | jq -r .principalId)
  ...
  printf '%-28s %-10s ' "$4" "$6"
  curl -s -H "Authorization: Bearer $1" -H 'Content-Type: application/json' "$SALES/v1/submissions" -d "$body" | jq -c '.error // "ok"'
}
tomorrow=$(date -u -v+1d +%F 2>/dev/null || date -u -d tomorrow +%F)
after=$(date -u -v+3d +%F 2>/dev/null || date -u -d '+3 days' +%F)
submit "$REP" crm-server seed-a1 crm.account.create crm.account ACME '{"name":"Acme Corp","kind":"company"}'
submit "$REP" crm-server seed-a2 crm.account.create crm.account HARBOUR '{"name":"Harbour Travel","kind":"company"}'
submit "$REP" crm-server seed-o1 crm.opportunity.open crm.opportunity OPP-1 '{"account":"ACME","title":"Board offsite"}'
submit "$REP" crm-server seed-o2 crm.opportunity.open crm.opportunity OPP-2 '{"account":"HARBOUR","title":"Winter group series"}'
submit "$REP" crm-server seed-b1 crm.opportunity.book crm.opportunity OPP-1 "{\"roomType\":\"suite\",\"checkIn\":\"$tomorrow\",\"checkOut\":\"$after\",\"guest\":\"Acme board\"}"
for i in 1 2 3 4; do # three standard rooms and one of overbooking: the fourth oversells the night
  submit "$MGR" hotel-server "seed-r$i" hotel.reservation.create hotel.reservation "R-10$i" \
