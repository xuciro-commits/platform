// Package hcm is human capital management (ADR-0025 D1, ADR-0017 D7), modelled
// on SAP SuccessFactors' absence management: people's leave requests,
...
// approval chain; the rules are the few only people management knows.
package hcm
...
const (
	ID           = "hcm"
	LeaveType    = "hcm.leave"
	SchemaCreate = "hcm.leave.create"
	SchemaSubmit = "hcm.leave.submit"
	SchemaCancel = "hcm.leave.cancel"
	Employee     = "employee"
	HR           = "hr"
...
func New(tenant string) *App {
	return &App{tenant: tenant, ledger: platform.NewLedger(tenant, ID, Actions(), LeaveType)}
}
