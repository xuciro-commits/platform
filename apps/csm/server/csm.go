// Package csm is customer service management (ADR-0025 D1, ADR-0021 D10 (2),
// ADR-0017's second proof), modelled on ServiceNow CSM: customers' tickets with a lifecycle, a service level kept by
...
type Ticket struct {
	platform.Record
	Number    string    `json:"number,omitempty" field:"readonly,search" help:"The ticket's number, given when it is opened: CS-2026-0001"`
	...
	Account   string    `json:"account,omitempty" field:"search" title:"Customer account"` // the CRM's, by ID; customer service knows no CRM
...
func New(tenant string) *App {
	return &App{tenant: tenant, ledger: platform.NewLedger(tenant, ID, Actions(), TicketType)}
}
...
func (a *App) Manifest() platform.Manifest {
	return platform.Manifest{Languages: languages, ID: ID, Title: "CSM", Version: "1", Actions: a.ledger.Catalog, Entities: Entities(),
		...
		Sequences: []platform.Sequence{{Name: "ticket", Pattern: "CS-{year}-{n:4}", Yearly: true}}}
}
...
func triager() platform.Agent {
	return platform.Agent{Name: "triage", Title: "Ticket triage",
		Instructions: `You triage and answer a customer's service ticket.
...
3. Triage the ticket: its category (billing, booking, technical, other) and priority (urgent, high, normal, low), with csm_ticket_triage.
4. Reply with csm_ticket_reply: short, polite, specific to what you found (name the stay or booking concerned, and the rule you rely on). Never promise refunds, discounts or compensation; say a colleague will follow up instead.
