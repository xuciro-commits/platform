...
const DATA_CLASS: &str = "pms.reservation";
...
fn draft(&mut self, schema: String, reservation_id: Option<String>, payload: Value, expected_revision: Option<u32>) -> Result<(), String> {
    ...
    let submission = Submission {
        ...
        schema: SchemaRef { name: format!("pms.reservation.{schema}"), version: 1 },
...
fn snapshot(&self) -> Value {
    ...
    let reservations = records("/v1/records/pms.reservation?sort=checkIn,id&archived=true&limit=500");
    let room_types = records("/v1/records/pms.room-type?sort=name");
...
/// Reproduces the slice's flows against a running pms-server started with
/// `-response-delay 4s -delay-count 1` (see apps/pms/flows.sh).
#[cfg(test)]
mod flows {
    use super::*;
    ...
    #[test]
    #[ignore = "needs a running pms-server; run apps/pms/flows.sh"]
    fn offline_timeout_conflict_and_rejection() {
        let server = std::env::var("HOTEL_SERVER").unwrap_or("http://127.0.0.1:8480".into());
        let dir = std::env::temp_dir().join(format!("pms-desk-{}", uuid::Uuid::new_v4()));
