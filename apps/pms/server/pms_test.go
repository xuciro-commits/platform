...
func newHotel() *Hotel {
	return hosted(New("hotel-a", map[string]RoomType{"standard": {Rooms: 1, Overbooking: 1}, "suite": {Rooms: 1}}))
}
...
func submission(p platform.Caller, schema, id, key string, payload any, expectedRevision ...uint32) *pb.Submission {
	...
	s := &pb.Submission{TenantId: p.Tenant, PrincipalId: p.ID, Authority: ID,
...
func hotelTenant(t *testing.T, journal *[]platformserver.Entry) (*Hotel, *platformserver.Tenant) {
	...
	h := New("hotel-a", map[string]RoomType{"standard": {Rooms: 1, Overbooking: 1}, "suite": {Rooms: 1}})
	tn, err := platformserver.NewTenant("hotel-a", platformserver.NewConsole("hotel-a",
		seat("desk-1", map[string]string{"pms": string(FrontDesk)}), seat("desk-2", map[string]string{"pms": string(FrontDesk)}),
		seat("manager-1", map[string]string{"pms": string(Manager), platformserver.PlatformApp: platformserver.Admin}),
		seat("channel-sim", map[string]string{"pms": string(Channel)})), h)
	if err == nil {
...
func TestChannelDuplicatesCollapse(t *testing.T) {
	...
	other := platform.Member{ID: "channel-2", Tenant: "hotel-a", Roles: map[string]string{"pms": string(Channel)}}
	raw, _ := json.Marshal(b)
...
func TestHotelUsesPlatformOperations(t *testing.T) {
	...
	member := func(id string) platform.Member {
		return platform.Member{ID: id, Tenant: "hotel-a", Roles: map[string]string{"pms": map[string]string{"desk-1": string(FrontDesk),
	...
	book := func(id, roomType, in, out string) string {
		...
		_, err := tn.Submit(member("desk-1"), &pb.Submission{TenantId: "hotel-a", PrincipalId: "desk-1", Authority: ID, IdempotencyKey: fmt.Sprint("b", keys),
	...
	set := func(name, value string) {
		...
		_, err := tn.Submit(member("manager-1"), &pb.Submission{TenantId: "hotel-a", PrincipalId: "manager-1", Authority: platformserver.PlatformApp,
			IdempotencyKey: fmt.Sprint("s", keys), Target: &pb.EntityRef{Type: platformserver.SettingType, Id: "pms/" + name},
...
func TestDrillE1ApartmentsAndCoworking(t *testing.T) {
	h := hosted(New("hotel-a", map[string]RoomType{
...
func as(id, tenant string, role Role) platform.Caller {
	return platform.As("pms", platform.Member{ID: id, Tenant: tenant, Roles: map[string]string{"pms": string(role)}})
}
