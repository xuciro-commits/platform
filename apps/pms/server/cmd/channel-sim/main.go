...
func main() {
	server := flag.String("server", "http://127.0.0.1:8480", "pms-server URL")
	...
	for i := 1; i <= *count; i++ {
		b := pms.ChannelBooking{MessageID: fmt.Sprintf("ota-%d", i), ReservationID: fmt.Sprintf("ota-%d", i),
			Guest: fmt.Sprintf("OTA Guest %d", i), SentAt: time.Now(),
			Stay: pms.Stay{RoomType: "suite", CheckIn: "2026-12-01", CheckOut: "2026-12-03"}}
