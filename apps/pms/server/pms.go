// Package pms is the property management system (ADR-0025 D1; #79), modelled
// on Oracle OPERA: room types, reservations taken at the desk or from a
// channel, and the lodging protocol it provides. It accepts reservation
// decisions through the kernel contract. It may
...
const (
	RoomTypeType    = "pms.room-type"
	ReservationType = "pms.reservation"
	ID              = "pms"
...
// New is a tenant's hotel whose room types start as rooms (keyed by ID).
func New(tenant string, rooms map[string]RoomType) *Hotel {
...
const channelMessageSchema = "pms.channel.booking"
...
func (h *Hotel) IngestChannelBooking(connector platform.Caller, b ChannelBooking, now time.Time) (*pb.ChangeRecord, *kernel.Error) {
	...
	record, err := h.Submit(connector, &pb.Submission{TenantId: h.tenant, PrincipalId: connector.ID, Authority: ID,
