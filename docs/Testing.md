...
3. **能自动走的路线就自动走**：第 4、5、17 到 21 条和第 23 条路线已写成浏览器测试（`web/e2e`，Playwright），`scripts/verify.sh web` 和 CI 每次都会跑；新路线走通后也补一个。
...
4. **请假审批**（酒店业主机，HCM）：
   ...
   3. 批完后请假变成 approved，`sales` 收到通知，点开就能看到这条审批（谁提交、谁批的）。审批人的待办通知在批准后自动变成已读；`sales` 开着的请假页面会自己刷新成"已批准"，不用刷新浏览器；
   4. 驳回时写上理由：请假变成"已驳回"，`sales` 收到带理由的通知，My requests 和请假页的"审批"一栏都能看到谁驳回、为什么；前面已经批过的人也会收到通知。已驳回的请假可以再次提交；
   5. 待审批期间不能取消，也不能重复提交；要改就在 My requests 里撤回，请假回到草稿。
5. **跨应用的决定和流程**（酒店业主机，ADR-0026）：
   ...
   3. 点"赢单"：酒店确认全部房间之前，商机仍是"进行中"、房块"确认中"；确认后房块变成"已确认"、商机变成"赢单"，酒店那边的保留变成"已预订"，商机动态里有"Hold confirmed"。酒店确认不了时房块变成"失败"，商机保持"进行中"，可以重新规划或者输单；
   ...
   7. 保留之后管理员在设置里把住宿协议换绑到另一个提供方：这间房的确认和释放仍然发给原来的酒店，新规划的房块才发给新的提供方。
   团队房块不走流程（ADR-0026 改成了协议里的保留），所以 CRM 这边没有流程实例；流程在工厂那边看：
   订单最后一个 SFC 完成后，"ERP 确认"流程自动发确认，ERP 的答复直接记在车间订单上；ERP 拒绝时主管收件箱里会有"修正并重发"的任务，重发后任务自动关闭。打开车间订单的记录页，"流程"一栏能看到这条确认流程和它的状态；`sup@plant.test` 在设置 → 流程 → 流程里看所有流程和实例，点开实例能看到每一步和原因，卡住的可以重试、跳过、取消。
...
22. **工作日和按状态启动**（ADR-0028）：
    1. 酒店业主机：请假审批的期限是提交人所在单位日历里的 2 个工作日，办公室周末和国庆（10 月 1 日到 7 日）不算，前台每天都算；收件箱里审批任务的到期时间能看到差别；
    2. 制造主机：车间订单最后一个 SFC 完成（或不合格品处置签字）后，订单变成"已完成"，"向 ERP 报工"流程自动启动，打开订单记录页的"流程"能看到它。
23. **导入、导出和委托审批**（ADR-0028）：
    1. 在任意记录列表（比如客户、ERP 产品）点"导出 CSV"：只导出你能看到的记录和字段，销售导出的商机里没有"预期毛利"；
    2. 改一改这个 CSV，或者新写一个（第一列 `id`，其余列是字段名），点"导入 CSV"：先预览每一行会新建还是修改、哪一行有错，确认后再导入；同一个文件再导一次不会重复；
    3. 酒店业主机：经理 `max` 用"委托审批"（`work.delegation.add`）把某几天的审批交给 `kim`；这几天里员工提交的请假，`kim` 的收件箱里有审批任务，批准后员工"我的申请"里的"代批"一栏写着 `kim 代 max`。
