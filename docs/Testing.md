...
3. **能自动走的路线就自动走**：第 4、5、17 到 21 条路线已写成浏览器测试（`web/e2e`，Playwright），`scripts/verify.sh web` 和 CI 每次都会跑；新路线走通后也补一个。
4. **失败也是路线的一部分**：被拒绝、超时、没人处理时，谁在哪里看到了什么，要写出来。
5. 自动检查是 `scripts/verify.sh`（见 AGENTS.md "Verify"）；本文是人要亲手走的部分。
...
4. **请假审批**（酒店业主机，HCM）：
   ...
   3. 批完后请假变成 approved，`sales` 收到通知，点开就能看到这条审批（谁提交、谁批的）。审批人的待办通知在批准后自动变成已读；`sales` 开着的请假页面会自己刷新成"已批准"，不用刷新浏览器。也可以试驳回、撤回，或者在审批期间取消请假（最后批准时会被拒绝并写明原因）。
5. **跨应用的决定和流程**（酒店业主机，ADR-0026）：
   1. `sales@hotel.test` 在 CRM 的客户页给一个商机点"规划团队住宿"：1 间 suite，填到达、离店，以及"保留至"（截止日，要早于到达）；
   2. 页面马上显示房块"已保留"，下面的住宿表里多一条酒店的保留（状态"已保留"）；酒店管理 → 预订里也能看到这间房处于保留状态；
   3. 点"赢单"：房块变成"已确认"，酒店那边的保留变成"已预订"，商机动态里有"Hold confirmed"；
   4. 给另一个商机规划同样日期的 suite：直接被拒绝（酒店只有一间），什么都不会记下；改成 2 间、换个日期：第一间保留成功、第二间被拒绝，房块变成"失败"，已保留的那间会自动释放。打开这个商机的记录页，能看到每间房的答复和历史；
   5. 输单时已保留的房间会被释放；截止日过了没赢单，酒店会自己释放，商机上的房块也随之变成"已释放"；
   6. 不接酒店也能走通（ADR-0025 D3）：没有住宿提供方时，规划后房间停在"已请求"，由人点"记录提供方的答复"逐间记录酒店电话或邮件里的答复；
   7. `manager@hotel.test` 在设置 → Processes → Flows 看所有流程和实例，点开实例能看到每一步和原因，卡住的可以重试、跳过、取消。
   工厂那边：订单最后一个 SFC 完成后，"ERP 确认"流程自动发确认，ERP 的答复直接记在车间订单上；ERP 拒绝时主管收件箱里会有"修正并重发"的任务，重发后任务自动关闭。打开车间订单的记录页，"流程"一栏能看到这条确认流程和它的状态。
...
17. **每个动作都有入口**（任一主机，#118）：
    1. 在 CRM → 客户、商机的列表页，右上角有"新建客户""新建商机"（平台按声明自动给出，应用不用自己写）；新建一个客户，再给它新建一个商机；
    2. 打开这个商机：页面上有"关闭商机""规划团队住宿""预订住宿"，点哪个就弹出按声明生成的表单（比如关闭商机要填 won 或 lost）；
    3. 另开一个浏览器窗口用别的身份改同一条记录，原来的页面会自己更新；
    4. 在 MES → 车间订单点"下达车间订单"，不接 ERP 也能下单（ADR-0025 D3）。


18. **持久任务的运行与健康**（任一主机，ADR-0027）：
    1. 以平台管理员登录，打开 设置 → 自动化：顶部显示租户健康——正常，或需要处理（各应用排队多少、最早等了多久、放弃了几项、哪个应用超出配额、哪个目的地的熔断器打开）；
    2. 在 集成 添加一个指向不存在地址的 Webhook（如 `http://127.0.0.1:9/hook`，勾"内部地址"），再做一个它订阅的操作：这个端点连续失败五次后熔断器打开，自动化页显示"熔断器打开"，其他端点照常发送；
    3. 工作最终放弃时（外发约七小时后、投递五次后），平台管理员会收到"… failed"通知，到 自动化 或 集成 里重试；
    4. `curl http://127.0.0.1:8495/healthz` 返回进程状态；给主机设 `OTEL_EXPORTER_OTLP_ENDPOINT` 后，链路和指标会导出到你的 OpenTelemetry 采集器，一次提交、它引起的流程和外发在同一条链路里。

19. **记录上的文件**（任一主机，ADR-0028）：
    1. 打开任意一条记录（比如客服工单、车间订单），在"文件"一栏点"添加文件"，选一个文件；
    2. 文件出现在列表里，点文件名就下载；能看这条记录的人都能下载，看不到这条记录的人下载不到；
    3. 文本或 Markdown 文件挂在知识文档上，就能被搜索和智能体引用；
    4. Docker 环境里文件存在 RustFS，管理界面在 `http://127.0.0.1:9001`（账号 `platform`，密码 `platform-files-local-only`）。

20. **字段权限和个人数据**（酒店业主机，ADR-0028）：
    1. 员工起草病假时填"医疗原因"：自己的请假页上看不到这个字段，搜索、筛选、分组也够不着它；人事能看到；
    2. 销售经理在商机上填"预期毛利"：经理看得到，负责这个商机的销售看不到也改不了；
    3. 人事看过带医疗原因的请假后，平台管理员在 设置 → 审计 的"个人数据的读取"里能看到谁在什么时候看了哪些记录的哪些字段。

21. **下拉选项、记录选择和评论**（任一主机，ADR-0028）：
    1. 在商机列表点"新建商机"：客户从下拉里选（只列你能看到的客户）；关闭商机时 won / lost 从下拉里选；
    2. 在任意记录页的"评论"里写评论，用 `@成员编号`（比如 `@manager-1`）提到别人：被提到的人收到通知；
    3. 评论后你自动关注这条记录，之后别人在这条记录上的操作和评论都会通知你；点"取消关注"就不再收到。