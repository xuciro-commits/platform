...
- **`apps/work`** (`work.New`, `work.ID`, `work.WorkTask`, `work.ApprovalRequest`): the host reaches it through the role `Tasks` (it serves `Caller.Assign` and closes tasks a flow or agent stops waiting for) and holds approval requests by routing to whichever app declares `work.approval.request`. `Host` grew what a platform app needs of the tenant: a member and the holders of an app role, an action's declaration, a caller for a member or for an app, a submission routed inside the input, recipients, the directory, and marking notifications seen. The host may use a platform app's types (its API description and context do); a platform app never imports the host.
- **`apps/flow`** (`flow.New`, `flow.ID`, `flow.FlowInstance`): the host reaches it through the role `Processes` (it declares every app's flows, pins the versions instances took for a replay, checks running instances at start, and hears an agent step's run end), and delivers it events as a `Listener`, the role the agent app takes too — the host's special cases for both are gone. A flow starts and signals agent runs through the role `Runs`, which the agent app takes; flow and agent no longer reach into each other.
- **`apps/ai`** (`ai.New`, `ai.ID`, `ai.Model`, `ai.Provider`, `ai.Usage`): providers, enabled models and metered usage. Calling a model stays in the host — it is the host's outbound I/O, with secrets and the journal of usage — and finds the app by the interface it needs (enabled models, providers, metering, a member's spend).
- **`apps/knowledge`** (`knowledge.New`, `knowledge.ID`, `knowledge.Document`, `knowledge.Term`): documents and the glossary. Searching stays in the host, which indexes every app's knowledge fields besides documents and calls embedding models; it reads the glossary through the interface it needs.
- **D4 amended (2026-09-26, the owner delegated #113):** the agent app and the console stay in the host. The agent app is the host's execution — model calls, tools over every app's reads and actions, search, the context graph, steps journaled one by one, evaluations re-running models — and the console is the configuration the host reads on every request (members, settings, endpoints, connectors, protocol bindings). Moving either would put nearly the whole host behind `internal/host`, which then bounds nothing. What the others take through roles, they reach directly; flows reach agents only through `Runs`.
- **Settings split by area:** `web/packages/platform/src` has one file per area — `people`, `apps`, `operations`, `ai`, `processes`, `knowledge` — and `shared` for the answer types and the administrator's host; `index.tsx` only declares the views and the navigation.
- **Proven (8c):** `scripts/verify.sh ci` and `web` at each move, the rehearsal after the last; every Settings area opened in the browser.