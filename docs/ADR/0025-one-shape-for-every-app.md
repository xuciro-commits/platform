...
| 8b | Industry names and one layout: apps, packages, IDs, entity types, web packages, solutions, Docker, the rehearsal and the test guide renamed; a development host and own tests for every app; the MES's hand-written web model replaced by the generated types; the scaffold writes the layout and `boundaries.sh` checks it | Every app's tests include a standalone scenario and `CheckReplay`; every app runs on its development host; the rehearsal passes on `hospitality-server` and `manufacturing-server`; the local stack reset and reseeded |
