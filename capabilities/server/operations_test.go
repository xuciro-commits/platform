...
func (p probe) Manifest() Manifest {
	m := p.notes.Manifest()
	m.Jobs = []Job{{Name: "tick", Title: "Tick", Every: time.Minute}}
	m.Settings = []Setting{{Name: "limit", Title: "Limit", Type: "integer", Default: "0"}}
	return m
}

var supervisors = Recipient{Structure: "site", Unit: "L1", Role: "supervisor"}

func (p probe) Input(c Caller, _ string, body []byte, now time.Time) (any, *kernel.Error) {
	if err := c.Deliver("p.topic", "", "", now); err != nil {
		return nil, err
	}
	c.Notify(Notification{Title: "Feed " + string(body), Ref: "p.topic/feed"}, now, supervisors)
	return nil, nil
}

func (p probe) Run(c Caller, _ string, now time.Time) *kernel.Error {
	if c.Setting("limit") == "0" {
		return nil
	}
	c.Notify(Notification{Title: "Tick", Key: "tick"}, now, supervisors, Recipient{Member: "ana"})
	return nil
}
...
func TestOperations(t *testing.T) {
	...
	build := func() *Tenant {
		dir := NewDirectory("t-1",
			...
			Seat{Subjects: []string{"gw"}, Member: Member{ID: "gw", Roles: map[string]string{}}})
		org := NewOrganization("t-1", OrgSeed{Structures: []Structure{{ID: "site", Name: "Sites", Kind: "site"}},
			Units: []Unit{{ID: "plant", Kind: "plant"}, {ID: "L1", Kind: "line"}},
	...
	n := 0
	decide := func(m Member, schema, typ, id, payload string) *kernel.Error {
		n++
	...
	inbox := func(m Member) []Notification {
